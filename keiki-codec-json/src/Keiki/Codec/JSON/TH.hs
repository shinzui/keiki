...
deriveRegFileCodecAs prefix tyName = do
  ...
  toEncSig  <- sigD toEncN
                 [t| $recTy -> Aeson.Encoding |]
  toEncDef  <- funD toEncN
                 [ clause [] (normalB
                     [| regFileToEncoding
                          @(RegFieldsOf $recTy)
                          . gToRegFile . Generics.from |])
  ...
  fromJSONSig <- sigD fromJSONN
                   [t| Aeson.Value -> Either String $recTy |]
  fromJSONDef <- funD fromJSONN
                   [ clause [] (normalB
...
validateRecord tyName = do
  ...
  case info of
    TyConI dec -> case dec of
      ...
      _ ->
        failure ("an unsupported declaration shape; only `data` and "
              <> "`newtype` are supported")
    _ ->
      failure ("not a type constructor; pass a record type name like "
            <> "''MyRecord")
  where
    failure :: String -> Q a
    failure detail =
...
validateCtors tyName ctors = case ctors of
  ...
  _              -> fail $ "deriveRegFileCodec: " <> show tyName
