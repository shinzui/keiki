...
spec = do
  describe "EP-22 M2: builder vs AST agreement on three canonical logs" $ do
    ...
    it "isFinal predicate matches across all eight vertices" $ do
      let vs = [Empty, OpenWithItems, Reserved, Paid,
                Shipped, Delivered, Cancelled, Refunded]
      [ isFinal orderCartAST v | v <- vs ] `shouldBe`
        [ isFinal orderCart    v | v <- vs ]

    it "edge counts per vertex match between forms" $ do
      let vs = [Empty, OpenWithItems, Reserved, Paid,
                Shipped, Delivered, Cancelled, Refunded]
      [ length (edgesOut orderCartAST v) | v <- vs ] `shouldBe`
        [ length (edgesOut orderCart    v) | v <- vs ]
  ...
  where
    ...
    walkBoth i (ev:rest) (sB, rB) (sA, rA)         =
      case (applyEvent orderCart sB rB ev, applyEvent orderCartAST sA rA ev) of
        ...
        (nB, nA) ->
          expectationFailure
            ("step " <> show i <> ": applyEvent disagreement: "
              <> "builder=" <> show (fmap fst nB)
              <> " ast="    <> show (fmap fst nA))
