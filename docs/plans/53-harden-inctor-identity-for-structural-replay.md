...
**Approach pivoted 2026-05-23** from a runtime `Typeable`/`eqTypeRep` schema comparison (Milestones 1–5 below, now SUPERSEDED) to a type-level **structural re-indexing** ("Design A-refined" — see Decision Log 2026-05-23 and the "Revised Plan of Work" section). The runtime-comparison approach was abandoned after it proved unsatisfiable for two polymorphic phantom `InCtor`s in `src/Keiki/Profunctor.hs` that flow through fixed-signature `Profunctor`/`Strong`/`Arrow`/`Choice` typeclass instances (no place to thread `Typeable`). The user chose the larger redesign.
...
- [x] ~~Add a focused regression test demonstrating the `icName` collision hazard.~~ (superseded 2026-05-23)
- [x] ~~Add `Typeable ifs` evidence to `InCtor` and compare `TInpCtorField` schemas via `eqTypeRep`.~~ (superseded 2026-05-23 — blocked by Profunctor instances)
- [x] ~~Remove the `unsafeCoerce` import/call from `src/Keiki/Core.hs`.~~ (folded into M-D)
- [x] ~~Update Haddock for the new replay identity rule / `icName` uniqueness.~~ (folded into M-D/M-J)
- [x] ~~Audit `mkInCtor`/`mkInCtorVia` for a new `Typeable ifs` constraint.~~ (no longer needed; the redesign requires no `Typeable`)
...
- **Implementation discovery (2026-05-23): the runtime-`Typeable` approach is unsatisfiable, falsifying the plan's "exactly five construction sites / near-zero risk / no API churn" premise.** Adding `Typeable ifs` to the `InCtor` GADT requires that evidence at *every* construction site. There are **seven** sites, not five. The two the prior validation missed are in `src/Keiki/Profunctor.hs`: `identityInCtor` (`:: forall a. InCtor a '[ '("payload", a) ]`, ~line 260) and `pairSndInCtor` (`:: forall ci c. InCtor (ci, c) '[ '("snd", c) ]`, ~line 574). Both build at a field schema mentioning a *polymorphic alphabet variable*, so they need `Typeable a` / `Typeable c`. They flow through `identityTransducer`, `arrTransducer`, and `firstSym` into the **`Profunctor`, `Strong`, `Arrow`, and `Choice` typeclass instances**, whose method signatures are fixed by `base`/`profunctors` and cannot carry a `Typeable` constraint. The compile failed with `No instance for Typeable c` / `Typeable a`. Forging the dictionary via `unsafeCoerce` (the module's existing `unsafeCoerceDisjointness`/`unsafeCoerceWrapperDict` trick) is *unsound* for `Typeable` — a bogus `TypeRep` would let `eqTypeRep` give wrong answers, re-introducing the very unsoundness being removed. Verified the contramap sites (`contraInCtor`, `leftInCtor`/`rightInCtor`) are fine — they pattern-match an existing `InCtor{}` and harvest its evidence — so only the two genuinely-polymorphic phantom constructors are blocked.

- **Discovery (2026-05-23): `TInpCtorField` is a fully general `Term`, not an output-only projection.** It is the public smart constructor `inpCtor = TInpCtorField` (`src/Keiki/Core.hs:634`) and appears in register updates (`USet`; see `updateReadsInput`), guards, output fields, the SBV translator (`translateTermSym`, `src/Keiki/Symbolic.hs:440`), weakening/substitution/lifting in `src/Keiki/Composition.hs`, and the contravariant rewriters in `src/Keiki/Profunctor.hs`. The authoring API (`inp`/`*:`/the TH `genTermFieldsRecord`) produces `Term`s. This rules out a separate output-only `OFInp` constructor (it would force changing the authoring API and the TH). The chosen design therefore re-parameterizes `Term` itself by `ifs` — see Decision Log "Design A-refined".

- **Discovery (2026-05-23): `firstSym` (Strong/Arrow) and `compose` deliberately mix input projections.** `firstSym` (`src/Keiki/Profunctor.hs:641`) prepends a `pairSndInCtor` projection (schema `'[ '("snd", c) ]`) onto the original output fields (a different schema) inside one `OutFields`. Re-indexing `OutFields` by a single `ifs` makes this ill-typed, so `firstSym` needs a rework: build a combined `InCtor (ci, c)` whose schema covers both the threaded `c` and the original fields, and shift the original fields' indices into it. `firstSym`'s `solveOutput` is already documented as dead (`firstWireCtor` sets `wcMatch = const Nothing`), so the rework is correctness-neutral. By contrast `compose`'s `substTerm`/`substOut` (`src/Keiki/Composition.hs:358`/`468`) already use `unsafeCoerce` for *value-type* alignment (unrelated to `ifs`); after substitution every projection in the composite `OPack` reads `t1`'s single input constructor, so the `ifs` threads cleanly there and the composite `OPack`'s `InCtor` and `OutFields` share `t1`'s existential `ifs` by construction.

- **Discovery (2026-05-23): the redesign requires no `Typeable` and (almost) no authoring change, but does change two TH-generated signatures.** Under Design A-refined, `gatherInpEntries`'s top-level `TInpCtorField` already has `ix :: Index ifs r` with `ifs` = the `OPack`'s schema, so it returns `ByIndex ix val` with no coercion; the `icName` equality is kept only as a runtime diagnostic for malformed edges. The TH `recordDecls` `inp<Short>` signature gains the `ifs` (`= RegFieldsOf payTy`) parameter, and the generated `<Short>TermFields rs ci` record (`genTermFieldsRecord`, `src/Keiki/Generics/TH.hs:508`) gains an `ifs` type parameter because its fields become `Term rs ci ifs ty`. Authors using record syntax / `*:` chains are unaffected (inference); only explicit `OutFields`/`Term`/`<Short>TermFields` type annotations in tests/jitsurei need the extra parameter.
...
- Decision: **Abandon the runtime `Typeable`/`eqTypeRep` comparison and implement the type-level structural re-indexing instead.**
  Rationale: During implementation the runtime-comparison approach failed to compile: `Typeable ifs` on the `InCtor` GADT cannot be supplied for the two polymorphic phantom `InCtor`s in `src/Keiki/Profunctor.hs` (`identityInCtor`, `pairSndInCtor`) that flow through the fixed-signature `Profunctor`/`Strong`/`Arrow`/`Choice` typeclass instances, and forging the dictionary with `unsafeCoerce` would be unsound (see Surprises 2026-05-23). When asked, the user chose the larger redesign the prior Decision Log entry had deferred. The redesign also turns out *cleaner*: it needs no `Typeable` anywhere, leaves the phantom `InCtor`s untouched, and makes the collision a compile error rather than a runtime `Nothing`.
  Date: 2026-05-23

- Decision: **Design A-refined — add an `ifs :: [Slot]` parameter to `Term`, expose it in `OutFields`/`OPack`, but existentially hide it in `Update`/`HsPred`.**
  Rationale: `TInpCtorField` is a general `Term` (used in updates, guards, output fields, the SBV translator, composition), so a separate output-only projection constructor is not viable (it would force changing the authoring API and the TH). Re-parameterizing `Term` by `ifs` — pinned by `TInpCtorField`, free on `TLit`/`TReg`, threaded through `TApp`/`TArith` — and exposing that `ifs` in `OutFields` lets `OPack :: InCtor ci ifs -> WireCtor co fields -> OutFields rs ci ifs fields -> OutTerm rs ci co` tie the projection schema to the constructor schema *by construction*. `gatherInpEntries` then needs no coercion. Crucially, `Update` (`USet`) and `HsPred` (`PEq`/`PCmp`) *existentially hide* the term's `ifs` (it appears only in arguments, not results), so `Update`, `HsPred`, `Edge`, and `SymTransducer` keep their current kinds and the change does not ripple into the transducer type or the `Edge`/`delta`/`omega` surface. `PEq`/`PCmp` take two *independent* existential `ifs` (maximally permissive; nothing requires the two operands to read the same constructor).
  Date: 2026-05-23

- Decision: **Keep the `icName` equality check in `gatherInpEntries` as a runtime diagnostic, not as type evidence.**
  Rationale: After re-indexing, a top-level `TInpCtorField` in an `OutFields` already has `ix :: Index ifs r` for the `OPack`'s `ifs`, so `ByIndex ix val` is sound with no check. The original code returned `Nothing` when the field's `InCtor` name differed from the `OPack`'s; preserving that `icName ic1 == icName ic2` guard keeps the malformed-edge diagnostic behavior identical for existing tests while the soundness now comes from the types.
  Date: 2026-05-23

- Decision: **Rework `firstSym` to a combined `InCtor (ci, c)`.**
  Rationale: `firstSym` prepends a `pairSndInCtor` projection (schema `'[ '("snd", c) ]`) onto the original fields (a different schema) in one `OutFields`; a single-`ifs` `OutFields` makes that ill-typed. Build a combined `InCtor (ci, c)` whose schema is `'( "snd", c)` consed onto the original `ifs`, project `c` from its head slot, and shift the original fields' indices into the combined schema. `firstSym`'s `solveOutput` is already dead (`wcMatch = const Nothing`), so this is correctness-neutral for forward processing. (Alternative considered: leave `firstSym` mixing schemas via a separate non-indexed path — rejected because it would re-introduce an `OutFields` element whose schema is not tied to the `OPack`, the exact hole this redesign closes.)
  Date: 2026-05-23
...
> **Revised 2026-05-23 — structural re-indexing (Design A-refined).** The milestone narrative below (Milestones 1–5) describes the *superseded* runtime-`Typeable` approach and is retained for context. The work actually being implemented is the structural re-indexing tracked by milestones **M-A … M-J** in the Progress section. Its shape:
>
> 1. `Term (rs :: [Slot]) (ci :: Type) (ifs :: [Slot]) (r :: Type)`. `TInpCtorField :: InCtor ci ifs -> Index ifs r -> Term rs ci ifs r` pins `ifs`; `TLit`/`TReg` leave it free; `TApp1`/`TApp2`/`TArith` thread it through subterms.
> 2. `OutFields (rs :: [Slot]) (ci :: Type) (ifs :: [Slot]) (fs :: Type)` with `OFCons :: Term rs ci ifs f -> OutFields rs ci ifs fs -> OutFields rs ci ifs (f, fs)`. `OPack :: InCtor ci ifs -> WireCtor co fields -> OutFields rs ci ifs fields -> OutTerm rs ci co` ties the two `ifs` (existential in `OutTerm`).
> 3. `USet :: KnownSymbol s => IndexN s rs r -> Term rs ci ifs r -> Update rs '[s] ci` and `PEq`/`PCmp` over `Term rs ci ifs r` *existentially hide* `ifs`; `Update`, `HsPred`, `Edge`, `SymTransducer` kinds are unchanged.
> 4. `gatherInpEntries` walks `OutFields rs ci ifs fs` against `InCtor ci ifs`; the top-level `TInpCtorField`'s `ix :: Index ifs r` is already at the `OPack`'s `ifs`, so it returns `ByIndex ix val` with **no `unsafeCoerce`** (keep the `icName` check as a runtime diagnostic). The colliding-schema term from the old Milestone-1 fixture is now a **compile error**, so the regression is a type-level guarantee plus a positive round-trip test.
> 5. Thread `ifs` through every `Term`/`OutFields` walker in `Core`, `Composition`, `Symbolic`, `Profunctor`, `Builder`, and the TH (`inp<Short>` signature + `<Short>TermFields` record gains an `ifs` param). Rework `firstSym` to a combined `InCtor (ci, c)`. Fix explicit `OutFields`/`Term`/`OutTerm` type annotations in tests/jitsurei.
> 6. Validate with `cabal test all`; confirm no `unsafeCoerce` remains in `gatherInpEntries`.

### Superseded narrative (runtime `Typeable` comparison)
