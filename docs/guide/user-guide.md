...
`Keiki.Composition` exports three combinators — `compose`,
`alternative`, and `feedback1` — for combining transducers
without leaving keiki's pure core. Pick by shape:

| Combinator | Shape | When to reach for it |
|---|---|---|
...
- `Disjoint (Names rs1) (Names rs2)` — register files must not
  share slot names. Compile-time `TypeError` if violated.
- `t1` outputs use `OPack` (the only output constructor today,
  so this is automatic).
- For `compose`: `t2`'s mid-side guards must be structural — no
  `TApp1`/`TApp2` escape hatches over the input — because the
  composition substitutes `t1`'s output term into `t2`'s
  mid-side reads.
...
and a "when to use this in event sourcing" section per
combinator. The formal semantics live in
...
| **`solveOutput`** | The *runtime* inverter on the replay path (called by `applyEvent`/`applyEventStreaming`/`reconstitute`): mechanically derives the inverse of `omega`/`evalOut` from each edge's `OutTerm`. The reason `applyEvent` exists at all without hand-written inverses. Which output shapes it can invert is the output-invertibility contract (`docs/guide/output-invertibility.md`). |
...
| **`SomeSymTransducer ci co`** | Existential wrapper around `SymTransducer (HsPred rs ci) rs s ci co` that hides `rs` and `s`. Lives in `Keiki.Profunctor`. The form ecosystem typeclasses (`Profunctor`, eventually `Category` / `Strong` / `Choice` / `Arrow`) expect. |
| **`someSymTransducer`** | Smart constructor for `SomeSymTransducer`; equivalent to applying the data constructor. |
| **`lmapCi` / `lmapMaybeCi`** | Pre-compose a transducer with a (partial) contramap on the input alphabet. Walks every embedded `InCtor` and rewrites its `icMatch`. Forward-only: rewritten edges' `solveOutput` is lossy by documented contract. See `docs/guide/profunctor.md`. |
| **`rmapCo`** | Post-compose a transducer with a covariant map on the output alphabet. Walks every embedded `WireCtor` and rewrites its `wcBuild`. Forward-only: rewritten edges' `wcMatch` becomes `const Nothing`. |
| **`dimapTransducer`** | One-shot bidirectional rewrite. Equivalent to `rmapCo g . lmapCi f`; identical variance caveat. |
...
| **`solveOutput`** | The *runtime* inverter that *solves* for the input given an event by inverting the edge's output term. The name describes the operation: solve the equation `output(input) = event` for `input`. |
...
- **Compose two aggregates.** `composition.md`. The smoke test
  ...
  semantics in
  `docs/research/composition-combinators-design.md`.
- **Reshape one aggregate's input or output alphabet.** `profunctor.md`.
  `lmapCi` / `rmapCo` / `dimapTransducer` / `lmapMaybeCi` rewrite
  the edges of an existing transducer to consume a wider command
  type, emit a versioned event, or filter incoming commands at the
  routing boundary. The wrapper `SomeSymTransducer ci co` slots
  keiki into ecosystem typeclasses (`Profunctor`, lens optics).
  Forward-only — read the variance caveat at the top of the
  guide before using it on a replay path.
