...
ocOps ::
  String ->
  SymTransducer
    (HsPred OC.OrderCartRegs OC.OrderCmd)
    OC.OrderCartRegs
    OC.OrderVertex
    OC.OrderCmd
    OC.OrderEvent ->
  ...
  [ bcompare (awkPath ["All", "UserReg", "builder", "step"]) $
      bench "UserReg/ast vs builder/step" $
        whnf
          ( step
              UR.userRegAST
              ( initial UR.userRegAST,
                initialRegs UR.userRegAST
              )
          )
          urCmd,
    ...
    bcompare (awkPath ["All", "OrderCart", "builder", "step"]) $
      bench "OrderCart/ast vs builder/step" $
        whnf
          ( step
              OC.orderCartAST
              ( initial OC.orderCartAST,
                initialRegs OC.orderCartAST
              )
          )
          ocCmd,
